...
sealed class EquationElement {
    ...
    operator fun plus(other: EquationElement): EquationElement {
        return when {
            ...
            // Preserve fontSize, color, and isBold from the first element
            this is Text && other is Text ->
                Text(
                    text = this.text + other.text,
                    fontSize = this.fontSize, // Keep fontSize from first element
                    verticalPadding = max(this.verticalPadding, other.verticalPadding),
                    horizontalPadding = max(this.horizontalPadding, other.horizontalPadding),
                    textColor = this.textColor, // Keep color from first element
                    isBold = this.isBold // Keep isBold from first element
                )
    ...
    data class Fraction(
        ...
        override fun draw(drawScope: DrawScope, topLeft: Offset) {
            ...
            // Always use the provided topLeft - don't center based on Canvas height
            // The calling code (MixedMathText) handles line positioning correctly
            // This ensures fractions align properly when in multi-line context
            val newTopLeft = topLeft
    ...
    data class Superscript(
        ...
        override fun draw(drawScope: DrawScope, topLeft: Offset) {
            ...
            // Always use the provided topLeft - don't adjust based on isMain
            // This ensures consistent alignment in multi-line context
            val newTopLeft = topLeft

            // Position the base element
            val topLeftForBase = Offset(
                x = newTopLeft.x,
                y = newTopLeft.y + scaledPowerHeight + verticalPadding
            )
